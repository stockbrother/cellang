...
import org.cellang.webframework.client.transferpoint.AbstractTransferProvider;
 ...
 * 
 */
public class AjaxTransferProvider extends AbstractTransferProvider {

	private static final WebLogger LOG = WebLoggerFactory.getLogger(AjaxTransferProvider.class);
	 ...
	 * @param wso
	 */
	public AjaxTransferProvider(Container c, Address uri) {
		super(uri);
	...
	protected void doRequest(final AjaxMsgWrapper am, final Handler<String> onfailure) {
		...
		JsonCallback jcb = new JsonCallback() {
			...
			public void onFailure(Method method, Throwable exception) {
				//
				// TODO
				AjaxTransferProvider.this.onRequestFailure(onfailure, am, method, exception); //
			}
			...
			public void onSuccess(Method method, JSONValue response) {
				...
				AjaxTransferProvider.this.onRequestSuccess(am, method, response);
				//
			}
		};
		m.send(jcb);
	...
	private void afterRequestSuccess(AjaxMsgWrapper am) {
		...
		this.scheduler.scheduleTimer(0, new Handler<Object>() {
			...
			public void handle(Object t) {
				AjaxTransferProvider.this.doSendHeartBeat(fsid);
			}
		});
		//
