...
import org.cellang.elastictable.operations.NodeQueryOperationI;
 ...
 * 
 */
public abstract class NodeQueryOperationSupport<O extends NodeQueryOperationI<O, W, R>, W extends RowObject, R extends ExecuteResult<R, ?>>
	 ...
	 * @param ds
	 */
	public NodeQueryOperationSupport(TableService ds, R rst) {
		super(rst);
	...
	protected BoolQueryBuilder buildQuery(R rst) {
		...
		// type field
		TermQueryBuilder typeQ = new TermQueryBuilder(TableRow.PK_TYPE, typeS);// NOTE,String
																			// is
																			// not
																			// string,cannot
																			// query
		...
		for (String key : mts.keyList()) {
			...
			qbi.type(MatchQueryBuilder.Type.PHRASE);
			qbi.slop(rg.slop);
			qbi.operator(MatchQueryBuilder.Operator.AND);//
			qb.must(qbi);//
		}
		// multi matches
	...
	public String getUniqueId() {
		//
		return (String) this.parameters.getProperty(TableRow.PK_UNIQUE_ID);
	}
	...
	public O id(String id) {
		//
		return this.propertyEq(TableRow.PK_ID, id);
	}
	...
	public O timestampRange(Date from, boolean includeFrom, Date to, boolean includeTo) {
		//
		this.propertyRange(TableRow.PK_TIMESTAMP, from, includeFrom, to, includeTo);
		return (O) this;
	}
