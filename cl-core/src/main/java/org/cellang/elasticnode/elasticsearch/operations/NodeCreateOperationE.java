...
import org.cellang.elasticnode.elasticsearch.UUIDUtil;
...
public class NodeCreateOperationE<W extends NodeWrapper> extends
	 ...
	 * @param ds
	 */
	public NodeCreateOperationE(NodeService ds) {
	...
	public String getId(boolean force) {
		return (String) this.properties().getProperty(Node.PK_ID, force);
	}
	...
	public void setId(String id) {
		this.properties().setProperty(Node.PK_ID, id);
	}
	/*
	 * Oct 27, 2012
	 */
	...
	protected void executeInternal(NodeCreateResultI rst) throws Exception {
		...
		pts.setProperty(Node.PK_UNIQUE_ID, uid);// uid field
		// id
		String id = this.getId(false);
		if (id == null) {
			id = UUIDUtil.randomStringUUID();
			pts.setProperty(Node.PK_ID, id);//
		}
		// timestamp
		pts.setProperty(Node.PK_TIMESTAMP, new Date());//
		// build properties
	 ...
	 */
	@Override
	public NodeCreateOperationI execute(NodeType type, HasProperties<Object> pts) {
		//
	...
	public static class ResultImpl extends ResultSupport<NodeCreateResultI, String> implements
		...
		public ResultImpl(NodeService ds) {
			super(ds);
		}
