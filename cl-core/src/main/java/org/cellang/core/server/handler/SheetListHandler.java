 ...
 * 
 */
public class SheetListHandler extends AbstracHandler {

	private static Logger LOG = LoggerFactory.getLogger(SheetListHandler.class);

	// protected ConfirmCodeNotifierI confirmCodeNotifier;

	// protected boolean isNeedConfirm = false;

	public SheetListHandler(TableService ts) {
		super(ts);
		/*
	...
	public void handle(MessageContext hc) {
		String owner = (String) hc.getRequestMessage().getPayload("owner", true);
		int max = 10000;
		List<SheetRowObject> rtX = this.tableService.getListNewestFirst(SheetRowObject.class, SheetRowObject.OWNER,
				owner, 0, max);
		List<HasProperties<Object>> rt = new ArrayList<HasProperties<Object>>();
		for (SheetRowObject so : rtX) {
			rt.add(so.getTarget());//
		}
