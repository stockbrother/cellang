/**
 *  Jan 31, 2013
 */
package org.cellang.webc.main.client;

import org.cellang.clwt.core.client.lang.Path;

/**
 * @author wuzhen
 * 
 */
public class HeaderItems {

	public static final Path H1_TOOLS = Path.valueOf("tools");

	public static final Path TOOLS_CONSOLE = Path.valueOf("tools/console");//

	public static final Path H1_USER = Path.valueOf("user");

	public static final Path USER_SIGNUP = Path.valueOf("user/signup");

	public static final Path USER_LOGIN = Path.valueOf("user/login");

	public static final Path USER_LOGOUT = Path.valueOf("user/logout");

	public static final Path CREATE_TABLE = Path.valueOf("create.table");

}
