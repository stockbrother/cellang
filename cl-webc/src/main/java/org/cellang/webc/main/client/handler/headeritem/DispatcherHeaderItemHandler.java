...
public class DispatcherHeaderItemHandler extends WebcHandlerSupport implements EventHandlerI<HeaderItemEvent> {
	...
	public DispatcherHeaderItemHandler(Container c) {
		...
		this.dispatcher.addHandler(HeaderItems.USER_SIGNUP, new UserSignupHeaderItemHandler(c));
		this.dispatcher.addHandler(HeaderItems.TOOLS_CONSOLE, new ConsoleHeaderItemHandler(c));
	}
	...
	public void handle(HeaderItemEvent t) {
		...
		LOG.debug("header itme event, header item path:" + p);
